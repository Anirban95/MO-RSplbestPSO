...
## Note: This code is referred form Zhao et al. Link is: https://github.com/ZhaoRichard/nproc
## The literature is with DOI: ""https://doi.org/10.1126/sciadv.aao1659"" is also cited in the literature 
...
class npc:
    ...
    # Kernel for MKL
    def build_k_kernels(self, omega1,omega2,omega3):

        def k_summer(_x1, _x2):
            #a linear Kernel
            b=0.5
            linear = _x1[:, np.newaxis] * _x2 + b
            norm1 = np.linalg.norm(linear, axis = 2)
            #a polynomial Kernel
            prod = (linear)**4
            norm2 = np.linalg.norm(prod, axis = 2)
            #a Radial Kernel
            norm3 = np.exp(np.square(np.linalg.norm(linear, axis = 2)/2))
            
            return omega1*norm1+omega2*norm2+omega3*norm3
        return k_summer
    
    def npc(self, x, y, kernel_summer, method, model = None, alpha = None, delta = None, split = None, split_ratio = None, n_cores = None, band = None, rand_seed = None):
        if alpha is None:
            alpha = 0.05
        if delta is None:
            delta = 0.05
        ...
        if split == 0:
            ...
            fits = self.npc_split(method, model,kernel_summer, x, y, p, alpha, delta, indices0, indices0, indices1, indices1, n_cores)
        else:
            # with split
            ...
            for i in range(split):
                ...
                if (band == True):
                    ...
                    fits.append(
                            self.npc_split(method, model,kernel_summer, x, y, p, alpha, delta, indices0train, indices0test,
        ...
        return [fits, split, split_ratio, errors]
        
        
    # NPC split
    # returns a fit
    def npc_split(self, method, model, kernel_summer, x, y, p, alpha, delta, indices0train, indices0test, indices1train, indices1test, n_cores):
        ...
        class_data = self.classification(method, model, x_train, y_train, x_test,kernel_summer)           
        if class_data == []:
            return []
    ...
    def classification(self, method, model, x_train, y_train, x_test, kernel_summer):
        
        if (method == "mkl"):
            fit_model = SVC(kernel= self.build_k_kernels(kernel_summer[0],kernel_summer[1],kernel_summer[2]), probability=True)
        else:
